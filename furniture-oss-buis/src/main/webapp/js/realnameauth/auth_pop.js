...
//	initJson.fileExt = "*.gif;*.jpg;*.jpeg;*.png;*.bmp";//图片格式
	...
	veUploadify(initJson, "file_upload1");
	 ...
	 $("#uploadPic1").bind("click",function(){
    ...
    $("#phoneCountryNo,#phoneCityNo,#phoneNo").blur(function() {
	    ...
	    $("#saveInfor_B").click(function(){
	    	...
	    	//$("form").submit();
			/**
			 * ajaxForm形式提交表单（弹出窗口需要ajax请求，并返回处理结果）
			 */
			$("form[name='realName']").ajaxSubmit({
				dataType:"json",
				success:function(data){
					if(data.success || data.success === "true"){
						//如果实名信息处理成功，返回审核结果（认证状态）
						closeWin(data.status);
					}else{
						//如果实名信息处理失败（即有错误信息），提示失败原因
						if(data.message !== ""){
							//会员信息未通过审核的提示信息
							$("#memeberInfoWarning").text(data.message);
						}
						if(data.errors !== "" && data.errors.length > 0){
							//错误提示信息
							var errorObjArray = data.errors;
							for(var i = 0 ;i < errorObjArray.length ; i++){
								$("#" +errorObjArray[i].field).closest("td").append('<span id="'+errorObjArray[i].field+'errors" class="errors">'+errorObjArray[i].defaultMessage+'</span>');
							}
						}
					}
				}
