...
function defaultOnComplete(event, ID, fileObj, response, data){debugger;
	...
	try{
		var filedPathObj = eval("get" + uploadFileId + "Path()");
		if(filedPathObj){
			$("#" + filedPathObj).val(rv.fileUrl);
		}
	}catch(e){
	}
	//如果有文件，则显示文件名称
...
function veUploadify(initJsonValue, uploadFileId){
	var jsonObj = {};
	jsonObj["uploader"] = "/js/uploadFile/uploadify.swf";
	jsonObj["script"] = "";
	//jsonObj["queueID"] = "fileQueue";
	//jsonObj["auto"] = false;
	jsonObj["cancelImg"] = "/js/uploadFile/cancel.png";
	//选择文件服务器
	...
	if(!isEmpty(initJsonValue, "buttonText")){
		...
		jsonObj["buttonImg"] = "/js/uploadFile/fileupload-browse.gif";
	}
	...
	try {
		...
		if(filedId){
			...
			if(!filedValue || parseInt(filedValue) < 0){
				...
				if(imgObj){
					var defaultImag = "/js/uploadFile/pic120.gif";
