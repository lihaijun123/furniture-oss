	...
	<table width="100%">
		...
		<tr>
			...
			<td>
				...
				<!-- 
				<c:if test="${category.catLevel eq 1 }">
				...
				<c:if test="${category.catLevel eq 2 }">
					<select id="parentCatCodeSel">
						<option value="-1">无</option>
						<c:forEach var="cate" items="${list }">
							<option value="${cate.catCode }" ${category.parent.catCode eq cate.catCode ? "selected='selected'" : ""} >${cate.catNameCn }</option>
						</c:forEach>
					</select>
				</c:if>
				<c:if test="${category.catLevel eq 3 }">
					<select id="parentCatCodeSel">
						<option value="-1">无</option>
						<c:forEach var="cate" items="${list }">
							<option value="${cate.catCode }" ${category.parent.parent.catCode eq cate.catCode ? "selected='selected'" : ""} >${cate.catNameCn }</option>
						</c:forEach>
					</select>
					<select id="parentCatCodeSel">
				...
				<c:if test="${category.catLevel eq 2 }">
					${category.parent.catNameCn }
				</c:if>
				<c:if test="${category.catLevel eq 3 }">
					${category.parent.parent.catNameCn } -> ${category.parent.catNameCn }
		...
		<tr>
			...
			<td >
				<input type="radio" name="catStatus" value="1" ${category.catStatus eq 1 ? "checked='checked'" : ""} /> 启用
				<input type="radio" name="catStatus" value="2" ${category.catStatus eq 2 ? "checked='checked'" : ""} /> 停用
			</td>
		</tr>
	</table>
