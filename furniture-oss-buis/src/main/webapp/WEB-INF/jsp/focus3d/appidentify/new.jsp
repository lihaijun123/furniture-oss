...
<form:form modelAttribute="appIdentifyFile" action="/appidentify.do" method="post">
	...
	<table width="100%">
		<caption class="x-panel-header">添加识别图高通文件</caption>
		<tr>
			<th>菜单类型：</th>
			<td >
				<select name="type">
					<c:forEach var="menuItem" items="${appMenuItemList }">
						<option value="${menuItem.type}">${menuItem.nameZh}</option>
					</c:forEach>
				</select>
			</td>
		</tr>
		<tr>
			...
			<td>
				...
				<input type="hidden" id="xmlFileSn" name="xmlFileSn"/><br>
				<span id="xmlFileSn_namedisplay" /><br>
			</td>
		</tr>
		<tr>
			<th>DAT文件：</th>
			<td>
				...
				<input type="hidden" id="datFileSn" name="datFileSn"/><br>
				<span id="datFileSn_namedisplay" /><br>
			</td>
		</tr>
	</table>
