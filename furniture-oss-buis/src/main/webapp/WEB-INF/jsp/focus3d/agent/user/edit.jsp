...
<form:form modelAttribute="agentUser" action="/agentUser.do" method="post">
	...
	<input type="hidden" name="regChannelType" value="${agentUser.regChannelType }" />
	<input type="hidden" id="userId" name="sn" value="${agentUser.sn }" />
	<font color="red">${message }</font>

	<table width="100%">
		<caption class="x-panel-header">修改微商代理</caption>
		<tr>
		...
		<tr>
			...
			<td >
				<select name="partnerType">
					<option value="1" ${agentUser.partnerType eq 1 ? "selected='selected'" : ""} >个人</option>
					<option value="2" ${agentUser.partnerType eq 2 ? "selected='selected'" : ""}>企业</option>
				</select>
			</td>
		</tr>
		<tr>
			...
			<td>
				...
				<select name="agentGrade">
					<c:forEach var="agentRole" items="${agentRoleList }">
						<option value="${agentRole.sn }" ${agentUser.agentGrade eq agentRole.sn ? "selected='selected'" : ""}>${agentRole.roleName }</option>
					</c:forEach>
				</select>
			</td>
		</tr>
		...
		<tr>
			...
			<td ><form:input path="mobilePhone"/><br><font color="red">手机号用于后面登录的账号</font></td>
		</tr>
		<tr>
			<th><font color="red">*</font>地区：</th>
			<td>
				<div style="float: left;">
					<select id="province" name="province" value1="${agentUser.province }">
           				<option></option>
           			</select>
           			<select id="city" name="city" value1="${agentUser.city }">
            			<option></option>
          			</select>
		...
		<tr>
			<th>手持身份证：</th>
			<td >
				<f:img id="idCardFileSn_img" hname="idCardFileSn" hid="idCardFileSn" width="120" height="120" src="${agentUser.idCardFileSn }" preview="true"/><br>
		...
		<tr>
			<th>企业营业执照：</th>
			<td >
				<f:img id="kbisFileSn_img" hname="kbisFileSn" hid="kbisFileSn" width="120" height="120" src="${agentUser.kbisFileSn }" preview="true"/><br>
		...
		<tr>
			<th>合同文件：</th>
			<td >
				<f:fileInfo hid="contractFileSn" hname="contractFileSn" id="contractFileSn_namedisplay" value="${agentUser.contractFileSn }"/>
				<br>
				<input id="file_upload3" name="file_upload3" type="file" /><br>
				<f:fileDownload name="" value="${agentUser.contractFileSn }"/>
			</td>
		</tr>
		<tr>
			<th>身份证号码：</th>
	...
	<c:if test="${agentUser.status eq 3 }">
		<table width="100%">
			...
			<tr>
				...
				<td>
						...
						<c:forEach var="userProduct" items="${userProductList }" varStatus="status">
							<input type="hidden" id="userProductList_${status.index}" value="${userProduct.sn }"/>
						</c:forEach>
				</td>
			</tr>
		</table>
...
<div style="text-align: center;margin: 10px 0px 10px 0px;">
	...
	<a id="resetPwd" href="javascript:void()">密码重置</a>|
	<a id="downloadsqs" href="javascript:void()" vhref="/agentUser.do?method=downloadsqs&sn=${agentUser.sn }">生成并下载授权书</a>
</div>
</body>
<!-- Ext Js Lib Start -->
